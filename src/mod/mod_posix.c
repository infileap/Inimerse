...
static void *g_native_handles[128];
...
static void load_one(VM *vm, const char *dir, const char *name) {
    char base[2048], p[2048];
    if (im_platform_path_join(base, sizeof base, dir, name) != 0 || im_platform_path_join(p, sizeof p, base, "mod.st") != 0) return;
    ...
    free(j);
    if (script) { if (im_platform_path_join(p, sizeof p, base, script) == 0) vm_exec_script_file(vm, p); free(script); }
    if (native && g_native_count < (int)(sizeof g_native_handles / sizeof g_native_handles[0])) {
        if (im_platform_path_join(p, sizeof p, base, native) == 0) {
            void *handle = dlopen(p, RTLD_NOW | RTLD_LOCAL);
            if (handle) {
                void (*init)(VM *) = (void (*)(VM *))dlsym(handle, "mod_init");
                if (init) { init(vm); g_native_handles[g_native_count++] = handle; }
                else dlclose(handle);
            }
        }
    }
