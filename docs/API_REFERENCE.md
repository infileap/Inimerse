...
语法糖分为两层：内糖由核心前端直接处理（区间、分号容忍、`++/--`、尾逗号、f-string、`|>`、集合推导）；外糖由脱糖层处理（`fn`、`print`、`&&/||`、`//`、`unless`、`say@target`）。V0.4 已确定但仍待 parser/compiler 接入的函数式内糖包括 `?`、lambda 和 `>>`，详见 [SYNTAX_SUGAR.md](SYNTAX_SUGAR.md)。
