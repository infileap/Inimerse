...
Hub 包分发：`POST /package` 接收 `{id,data}`（Base64 `.vvpkg`），`GET /package/<id>` 下载包；客户端对应 `publishPackage(id, bytes)` 与 `downloadPackage(id, {hash, maxBytes})`。下载端会校验大小和可选 SHA-256 摘要。
