...
class UppSession {
  ...
  apply(message) {
    ...
    if (message.type === 'heartbeat') { if (!Number.isSafeInteger(p.seq) || p.seq < this.lastHeartbeat) throw new Error('heartbeat sequence out of order'); this.lastHeartbeat = p.seq; this.lastHeartbeatAt = Number.isFinite(p.timestamp) ? p.timestamp : Date.now(); return this.state; }
